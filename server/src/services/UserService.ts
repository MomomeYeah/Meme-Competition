...
export class UserService {
    static async register(req: AuthRequest): Promise<{ userId: string; token: string; user: User }> {
        ...
        return { userId, token, user: user as User };
    }

    static async login(username: string, password: string): Promise<{ userId: string; token: string; user: User }> {
        ...
        const { passwordHash: _, ...user } = doc;
        return { userId: doc.id, token, user: user as User };
    }
